...
!function(){$(document.body).on("azbn.url-history.init",null,{},function(t){t.preventDefault(),$("._msp__block .scroller").mCustomScrollbar(),$(".scroller").mCustomScrollbar(),$(".twentytwenty-container").twentytwenty(),$(document.body).trigger("azbn.own.init"),$(window).trigger("resize")}),$(document.body).on("azbn.url-history.get",null,{},function(t,e,n,o){t.preventDefault(),$.get(e,{},function(t){var l=$("<div/>",{html:t});if($(n).html(l.find(n).eq(0).html()).attr("class",l.find(n).eq(0).attr("class")),l.empty().remove(),$('.azbn-load-link[href="'+e+'"]').length)$('.azbn-load-link[href="'+e+'"]').closest("li").addClass("active");else{var r=window.location.href;r=r.split("/").pop(),$('.azbn-load-link[href="./'+r+'"]').closest("li").addClass("active")}1==o&&window.history.pushState({href:e,target:n},null,e),$(document.body).trigger("azbn.url-history.init")})}),window.addEventListener("popstate",function(t){t.state&&t.state.target&&$(document.body).trigger("fecss.url-history.get",[window.location.pathname,t.state.target,!1])},!1)}();
